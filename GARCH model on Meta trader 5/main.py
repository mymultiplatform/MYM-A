...
from data_processing.data_functions import start_garch_analysis
...
def main():
    ...
    connect_button = tk.Button(login_frame, text="Connect", font=("Helvetica", 14),
                               command=lambda: connect_and_run(login_entry.get(), password_entry.get(), server_entry.get()))
...
def connect_and_run(login, password, server):
    ...
    if connect_to_mt5(login, password, server):
        update_status("Connected. Starting GARCH analysis...")
        start_garch_analysis()
    else:
