...
def perform_backtest():
    ...
    for i in trade_positions:
        ...
        # Set exit condition without look-ahead bias
        trade_open = True
        for j in range(i+1, len(data)):
            next_price = data['close'].iloc[j]

            # Simulate real-time sequential price check
            if trade_open:
                if trade_type == 'Buy' and next_price >= open_trade['entry_price'] + 99.9 / lot_size:
                    # Exit for Buy when profit condition is met
                    ...
                    # Exit for Sell when profit condition is met
