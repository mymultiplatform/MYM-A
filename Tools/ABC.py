...
def train_lstm_model(scaled_data):
    ...
    model.train()
    for epoch in range(50):  # Increased epochs for better training
        optimizer.zero_grad()
...
def predict_future(model, data, future_days):
    ...
    with torch.no_grad():
        for _ in range(future_days):
            input_seq_tensor = torch.tensor(input_seq.reshape((1, input_seq.shape[0], 1)), dtype=torch.float32)
            predicted_price = model(input_seq_tensor).item()
            predictions.append(predicted_price)
            input_seq = np.append(input_seq[1:], predicted_price)  # Update input sequence
    return predictions
